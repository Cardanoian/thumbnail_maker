...
def test_photo_stays_within_cap(tmp_path: Path) -> None:
    ...
    document = pdfium.PdfDocument(result.pdf_bytes)
    try:
...
def test_korean_filename(tmp_path: Path) -> None:
    ...
    result = convert(source)
    output = save_pdf(default_output_path(source), result.pdf_bytes)
    assert output.name == "여행 사진_20kb.pdf"
...
def test_ensure_cap_rejects_oversize() -> None:
    with pytest.raises(TooLargeError):
        ensure_size_cap(b"%PDF" + b"x" * MAX_BYTES)


def test_save_pdf_does_not_leave_oversize_file(tmp_path: Path) -> None:
    target = tmp_path / "out.pdf"
    with pytest.raises(TooLargeError):
        save_pdf(target, b"%PDF" + b"x" * MAX_BYTES)
...
def test_saved_file_size_never_exceeds_cap(tmp_path: Path) -> None:
    ...
    result = convert(source)
    output = save_pdf(tmp_path / "doc_20kb.pdf", result.pdf_bytes)
    assert output.stat().st_size <= MAX_BYTES
